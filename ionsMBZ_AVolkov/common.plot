...
logfilename = sprintf ("logs/%02d%s%+d.log", Z0, name, charge)
pdffilename = sprintf ("pdfs/%02d%s%+d.pdf", Z0, name, charge)
...
plot [0:xmax] \
	datafile \
	usi 1:(mby($1, $2)) ti name.", Mott-Bethe-Yonekura", \
...
sfac = sprintf ("SFAC %2s%+d %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f =\n".\
	"   %3.1f %3.1f %.3f %.3f %.3f", \
        name, charge, ca1, cb1, ca2, cb2, ca3, cb3, ca4, cb4, cc, fp, fpp, mu, r, wt)
...
stats [xmin:xmax] datafile usi 1:(abs(mby($1, $2) - cm($1))) nooutput prefix 'Num'

stats [xmin:xmax] datafile usi (abs(mby($1, $2))) nooutput name 'Denom'

Rscat = Num_sum_y/Denom_sum
set print 
print (sprintf("Rscat for %d%s%+d after fitting: %4.4f%% (%4.3f / %4.3f)\n", Z0, name, charge, 100.*Rscat, Num_sum_y, Denom_sum))
set print "Rscat_values.log" append
print (sprintf("Rscat for %d%s%+d after fitting: %4.4f%% (%4.3f / %4.3f)\n", Z0, name, charge, 100.*Rscat, Num_sum_y, Denom_sum))

plot [xmin:xmax] \
	datafile \
	usi 1:(mby($1, $2)) ti sprintf("%s^{%+d}", name, charge), \
	cm(x) ti "Cromer-Mann fit"
...
plot [xmin:xmax] \
	datafile \
	usi 1:(cm(column(1))-(mby($1, $2))) noti w lp
...
plot [xmin:xmax] \
	datafile \
	usi 1:((cm(column(1))-mby($1, $2))/cm(column(1))) noti w lp
